...
public class DisplayEntity extends Entity implements DragAndDropable {
	...
	public void dragged(int x, int y, Vec3d newPos) {
		// Normal objects
		if (!usePointsInput()) {
			InputAgent.applyVec3d(this, positionInput.getKeyword(), newPos, DistanceUnit.class);
			return;
		}
	...
	@Output(name = "EntityDependentList",
	 description = "The entities whose inputs include one or more references to this entity.",
	    sequence = 12)
	public ArrayList<DisplayEntity> getEntityDependentList(double simTime) {
		ArrayList<DisplayEntity> ret = new ArrayList<>();
		for (DisplayEntity ent : getJaamSimModel().getClonesOfIterator(DisplayEntity.class)) {
			if (ent == this)
				continue;
			if (ent.getEntityReferences().contains(this)) {
				ret.add(ent);
			}
		}
		return ret;
